...
void bottom_up_radix_sort(Range in, Range out,
                          long num_bits, long bot_bit, bool inplace) {
  const long radix_bits = 8;
  if (bot_bit >= num_bits) {
    if (!inplace) parlay::copy(in, out);
  } else {
    long num_buckets = std::min(1l << radix_bits,
                                1l << (num_bits - bot_bit));
    ...
    counting_sort(in.begin(), in.end(), out.begin(), keys.begin(),
                  num_buckets);
    bottom_up_radix_sort(out, in,
                         num_bits, bot_bit + radix_bits, !inplace);
  }
}
...
void radix_sort(Range in, Range out, long bits, bool inplace) {
  ...
  if (n < cutoff) 
    ...
    long radix_bits = std::min({bits,
                                10l,
                                (long) std::ceil(std::log2(2*n/cutoff))});
    ...
    // sort in into out based on keys
...
// An inplace (i.e., result is in the same place as the input)
// integer_sort.  Requires O(n) temp space.
template <typename Range>
void integer_sort(Range& in, long bits) {
  using V = typename Range::value_type;
  auto tmp = parlay::sequence<V>::uninitialized(in.size());
