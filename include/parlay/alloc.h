...
#ifndef PARLAY_ALLOC_H_
#define PARLAY_ALLOC_H_
...
struct pool_allocator {
 ...
 public:
  ...
  pool_allocator(std::vector<size_t> const& sizes) : sizes(sizes) {
    ...
    small_allocators = (struct block_allocator*)malloc(
        num_buckets * sizeof(struct block_allocator));
    ...
    for (size_t i = 0; i < num_small; i++) {
      ...
      if (bucket_size < 8)
        throw std::invalid_argument(
            "for small_allocator, bucket sizes must be at least 8");
      if (!(bucket_size > prev_bucket_size))
        throw std::invalid_argument(
            "for small_allocator, bucket sizes must increase");
  ...
  void reserve(size_t bytes) {
    ...
    parallel_for(0, bc,
                 [&](size_t i) { h[i] = allocate(small_alloc_block_size); }, 1);
    parallel_for(0, bc,
                 [&](size_t i) {
                   for (size_t j = 0; j < small_alloc_block_size;
                        j += (1 << 12))
                     ((char*)h[i])[j] = 0;
                 },
                 1);
    for (size_t i = 0; i < bc; i++) deallocate(h[i], small_alloc_block_size);
  }
  ...
  void print_stats() {
    ...
    std::cout << "Total bytes allocated = " << total_a + large_allocated
              << std::endl;
