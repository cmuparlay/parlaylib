...
template <class Seq, class Compare>
typename Seq::value_type kth_smallest(Seq const &S, size_t k,
				      Compare less,
				      random r = random()) {
  // raise exception if empty sequence?
  using T = typename Seq::value_type;
  ...
  parlay::sequence<T> samples(num_samples, [&] (auto i) {
      return S[r[i] % n]; });
  auto s = sample_sort(samples, less);
  ...
  if (k == l) return a;
    ...
    return kth_smallest(filter(s, [&] (T x) { return less(x, a);}),
			k, less, r.next());
  else if (k == h) return b;
  else if (k > h) {
    auto m = count_if(S, [&] (T x) {return !less(a, x);});
    if (k < m) return a;
    return kth_smallest(filter(s, [&] (T x) { return less(a, x);}),
			k-m, less, r.next());
  } else { // k is between l and h
    if (!less(a,b)) return a;
    // the common case
    auto x = filter(s, [&](T c) { return !less(c, a) && less(c,b);});
    return sample_sort(x, less)[k - l];
  }
}
