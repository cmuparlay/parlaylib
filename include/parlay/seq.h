...
#ifndef PARLAY_SEQ_H_
#define PARLAY_SEQ_H_
...
struct sequence {
 public:
  ...
  sequence(const sequence& a) {
    if (report_copy && !a.is_small())
      std::cout << "copy constructor: len: " << a.size()
                << " element size: " << sizeof(value_type) << std::endl;
    if (a.is_small())
      val = a.val;
    else
      copy_from(a.val.large.s, a.val.large.n);
  }
  ...
  sequence(const size_t sz) { alloc(sz); }
  ...
  // template <class Seq, typename
  // std::enable_if_t<!std::is_integral<Seq>::value>>
  ...
  value_type& get(const size_t i) const { return begin()[i]; }
 ...
 private:
  ...
  inline bool is_small(size_t sz) const {
    return ((T_size <= max_sso_size) && sz < (lg_size / T_size) && sz > 0);
  }
  ...
  void initialize_elements() {
    if (!std::is_trivially_default_constructible<value_type>::value)
      parallel_for(0, size(),
                   [&](size_t i) { new ((void*)(begin() + i)) value_type; });
  }

  void delete_elements() {
    if (!std::is_trivially_destructible<value_type>::value)
      parallel_for(0, size(), [&](size_t i) { (begin() + i)->~value_type(); });
  }
...
auto to_sequence(Seq const& s) -> sequence<typename Seq::value_type> {
  ...
  return sequence<T>(s.size(), [&](size_t i) { return s[i]; });
}
...
std::ostream& operator<<(std::ostream& os, sequence<char> const& s) {
  // pad with a zero
  sequence<char> out(s.size() + 1,
                     [&](size_t i) { return i == s.size() ? 0 : s[i]; });
  os << out.begin();
  return os;
}
