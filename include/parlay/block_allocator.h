...
struct block_allocator {
 ...
 public:
  ...
  ~block_allocator();
  block_allocator(size_t block_size, size_t reserved_blocks = 0,
                  size_t list_length_ = 0, size_t max_blocks_ = 0);
  block_allocator(){};
};
...
void block_allocator::print_stats() {
  ...
  std::cout << "Used: " << used << ", allocated: " << allocated
            << ", block size: " << size << ", bytes: " << size * allocated
            << std::endl;
}

block_allocator::block_allocator(size_t block_size, size_t reserved_blocks,
                                 size_t list_length_, size_t max_blocks_) {
  blocks_allocated = 0;
...
void block_allocator::clear() {
  if (num_used_blocks() > 0)
    std::cout << "Warning: not clearing memory pool, block_size="
              << block_size() << " : allocated blocks remain" << std::endl;
  else {
    // clear lists
    for (int i = 0; i < thread_count; ++i) local_lists[i].sz = 0;
